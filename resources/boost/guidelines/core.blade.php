...
The `misaf/vendra-affiliate-api` package exposes the public referral lookup surface for `misaf/vendra-affiliate` through API Platform for Laravel.
...
- Treat the API as public. Referral-code lookup is read-only and exposes active affiliates only, returning just the referral code and creation timestamp; never expose users, commission terms, payouts, or other financial data. The only write is the public, throttled, validated referral-visit endpoint (`POST /marketing/affiliate-clicks`), which delegates to the `misaf/vendra-affiliate` domain action, returns `204`, and serializes no output.
